...
from po_config import MODEL_DIR

# import utils

parser = argparse.ArgumentParser()
parser.add_argument(
    "-llvm", "--llvm_dir", required=True, help="path to llvm-build directory"
)
parser.add_argument(
    "-ir2vec",
    "--ir2vec_dir",
...
parser.add_argument(
    ...
    "--train_dir",
    required=True,
    help="path to directory with LLVM IR files for training",
)
parser.add_argument(
    "-iter", "--train-iterations", required=False, type=int, default=300
)
parser.add_argument(
    "-a", "--isAArch", required=False, default=False, action="store_true"
)
parser.add_argument(
    "-log", "--log_dir", required=False, type=str, default="0.2thresh-10alpha-5beta-x86"
)
