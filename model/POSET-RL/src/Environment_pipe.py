...
class PhaseOrder(gym.Env):
    def __init__(self, config):
        ...
        self.clang_arch_flag = (
            "-mcpu=cortex-a72" if config["target"] == "AArch64" else ""
        )
        self.opt_arch_flag = (
            "--mcpu=cortex-a72" if config["target"] == "AArch64" else ""
        )
        ...
        self.Obs = None
        obs_space = Box(
            FLOAT_MIN, FLOAT_MAX, shape=(config["state_size"],), dtype=np.float32
        )
        self.observation_space = Dict(
            {
                "action_mask": Box(0, 1, shape=(self.action_space_size,)),
                "state": obs_space,
            }
        )
        ...
        if self.mode != "inference":
            ...
            self.test_Benchmark = os.path.join(self.FileSys_Obj.PhaseOrderDir, "")

        self.assembly_file_path = (
            f"{self.temporaryDirectory}/assemblyfile_{self.worker_index}.s"
        )
        ...
        if self.use_pipe:
            # self.temp_rootname = "/tmp/" + config["pipe_name"]
            self.compiler_interface = PipeCompilerInterface(
                self.data_format, self.temp_rootname
            )
    ...
    def createEnv(self, fileName):
        ...
        if self.mode != "inference":
            self.FileSys_Obj.copyFile(
                os.path.join(self.FileSys_Obj.TrainingDataPath, fileName), self.Curr_Dir
            )
        else:
            # quiet#            print("test_Benchmark {}".format(self.test_Benchmark))
            logging.info("test_Benchmark {}".format(self.test_Benchmark))
            self.FileSys_Obj.copyFile(
                os.path.join(self.test_Benchmark, fileName), self.Curr_Dir
            )
        ...
        if self.mode != "inference":
            ...
            if len(self.Obs) >= 1:
                ...
                if self.use_grpc and self.compiler_interface is None:
                    self.compiler_interface = GrpcCompilerInterface(
                        mode="client",
                        stub_class=posetRL_pb2_grpc.PosetRLServiceStub,
                        hostip="127.0.0.1",
                        hostport=self.server_port,
                    )
    ...
    def getBinarySize(self, IRFile, init=False):
        ...
        if init:
            ...
            command = (
                self.FileSys_Obj.OptPath
                + " "
                + self.opt_arch_flag
                + " -S  -add-size-attr --enableMinSizeAttr --removeNoInlineAttr "
                + self.Curr_Dir
                + "/"
                + fileName
                + ".ll -o "
    ...
    def step(self, action_index):
        ...
        next_observation = {
            "action_mask": np.array(self.cur_action_mask),
            "state": self.embedding,
        }
        self.cur_obs = next_observation
    ...
    def getMCACost(self, new_file):
        ...
        os.system(cmd1)
        cmd2 = (
            self.FileSys_Obj.MCAPath + " " + self.opt_arch_flag + " " + new_file + ".s"
        )
    ...
    def getReward(self, AssemblyFilePath):
        ...
        llvmMcaCommand = (
            f"{self.FileSys_Obj.MCAPath} {self.opt_arch_flag} {AssemblyFilePath}"
        )
        pro = subprocess.Popen(
            llvmMcaCommand,
            executable="/bin/bash",
        ...
        if self.lastMcaThroughtput is None:
            mca_cost = (
                self.OzMcaThroughtput - currMcaThroughtput
            ) / self.OzMcaThroughtput
        else:
            mca_cost = (
                self.lastMcaThroughtput - currMcaThroughtput
            ) / self.OzMcaThroughtput
